...
public:
    ...
    void print() {
    cout << "Vista de la matriz de adyacencia con pesos:\n";
...
void kruskal() {
    ...
    cout << "\nConexiones seleccionadas en el Árbol de Expansión Mínima (Kruskal):\n";
    for (int i : indices) {
        int u = edges_u[i];
        int v = edges_v[i];
        ...
        if (uf.union_sets(u, v)) {
            cout << "Nodo " << u << " unido con Nodo " << v << " (costo: " << w << ")\n";
            mst_weight += w;
        }
    }
    cout << "Suma total de costos del árbol mínimo generado: " << mst_weight << endl;
}
};
